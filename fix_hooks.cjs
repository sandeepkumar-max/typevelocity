...
const hookString = `  // SEO & Routing Effect
  useEffect(() => {
...
// Remove the wrongly injected code and restore `case 'home': return (`
const originalWrongChunk = `      case 'home': \n` + hookString + `\n  return (`;

if (code.includes(originalWrongChunk)) {
  code = code.replace(originalWrongChunk, `      case 'home': return (`);
  // Inject the hook string right before const renderContent = () => {
  code = code.replace(
    "const renderContent = () => {",
    hookString + "\n\n  const renderContent = () => {"
  );
  fs.writeFileSync('src/App.tsx', code);
  console.log("Fixed hooks placement!");
} else {
  console.log("Could not find the exact chunk to replace");
}
